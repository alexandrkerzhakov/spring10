package ru.gb.diplom.controller.virusTotal;
...
import ru.gb.diplom.model.virusTotal.*;
import ru.gb.diplom.properties.VirusTotalProperties;
import ru.gb.diplom.service.virusTotal.VirusTotalEntityService;
...
public class VirusTotalController {
    ...
    @GetMapping("/ip/{ip}/votesEntity")
    public String getVotesEntityOnIp(@PathVariable("ip") String ip, Model model) {

        Request request = virusTotalService.createGetRequest(virusTotalProperties.getBASE_URL(), "ip_addresses/", ip, "/votes");
        String json = virusTotalService.getResponseString(request).orElseThrow();
        Votes votes = virusTotalService.getInstance(json, Votes.class);
        model.addAttribute("ipVotesInstance", votes);

        return "virus_total/ip_votes2";
    }

    @PostMapping("/saveIpVotes")
    public String saveIpVotes(@ModelAttribute("ipVotesInstance") Votes votes) {
        virusTotalEntityService.save(votes);
        System.out.println(votes);
        return "redirect:/virusTotal/all";
    }

    @GetMapping("/all")
    public String findByAll(Model model) {
        List<Votes> votesList = virusTotalEntityService.findAll();
        model.addAttribute("votesList", votesList);
        return "virus_total/allVotes";
    }
